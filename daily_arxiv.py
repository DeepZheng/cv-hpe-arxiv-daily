...
def get_daily_papers(topic, query="pose", max_results=2):
    """
    @param topic: str
    @param query: str
    ...
    # output
    content = dict()
    monthly_content = dict()
    content_to_web = dict()
    monthly_content_to_web = dict()
    ...
    for result in search_engine.results():
        ...
        update_month = str(update_time.year) + "-" + str(update_time.month)
        ...
        try:
            ...
            if repo_url is not None:
                monthly_content[paper_key] = "|**{}**|**{}**|{} et.al.|[{}]({})|**[link]({})**|\n".format(
                    update_time, paper_title, paper_first_author, paper_key, paper_url, repo_url)
                monthly_content_to_web[paper_key] = "- {}, **{}**, {} et.al., Paper: [{}]({}), Code: **[{}]({})**".format(
                    update_time, paper_title, paper_first_author, paper_url, paper_url, repo_url, repo_url)

            else:
                monthly_content[paper_key] = "|**{}**|**{}**|{} et.al.|[{}]({})|null|\n".format(
                    update_time, paper_title, paper_first_author, paper_key, paper_url)
                monthly_content_to_web[paper_key] = "- {}, **{}**, {} et.al., Paper: [{}]({})".format(
                    update_time, paper_title, paper_first_author, paper_url, paper_url)

            # TODO: select useful comments
            comments = None
            if comments != None:
                monthly_content_to_web[paper_key] += f", {comments}\n"
            else:
                monthly_content_to_web[paper_key] += f"\n"
            
            content[update_month] = monthly_content
            content_to_web[update_month] = monthly_content_to_web
...
def update_json_file(filename, data_dict):
    ...
    for data in data_dict:
        for keyword in data.keys():
            papers = data[keyword]
            for update_month in papers.keys():
                if update_month in json_data.keys():
                    if keyword in json_data[update_month].keys():
                        json_data[update_month][keyword].update(papers[update_month])
                    else:
                        json_data[update_month][keyword] = papers[update_month]
                else:
                    json_data[update_month] = {}
                    json_data[update_month][keyword] = papers[update_month]
            ...
            # for paper in papers:
                # print(papers[paper])
                # json_data[update_month][keyword] = paper
...
def json_to_md(filename, md_filename,
    ...
    with open(md_filename, "a+") as f:
        ...
        if use_tc == True:
            f.write("<details>\n")
            f.write("  <summary>Time Line</summary>\n")
            for month in data.keys():
                month_data = data[month]
                if not month_data:
                    continue
                kw_month = month.replace(' ', '-')
                f.write(" <details>\n")
                f.write(f"    <summary><a href=#{kw_month.lower()}>{month}</a></summary>\n")
                for keyword in month_data.keys():
                    day_content = month_data[keyword]
                    if not day_content:
                        continue
                    kw = keyword.replace(' ', '-')
                    f.write(f"    <ol><a href=#{kw.lower()}>{keyword}</a></ol>\n")
                f.write(" </details>\n")
